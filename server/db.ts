...
import { InsertUser, users, conversions, InsertConversion } from "../drizzle/schema";
...
export async function getUserConversions(userId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function updateConversionStatus(
  ...
  updates?: Partial<Omit<InsertConversion, 'id'>>
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  await db.update(conversions).set(updateData).where(eq(conversions.id, id));
}
